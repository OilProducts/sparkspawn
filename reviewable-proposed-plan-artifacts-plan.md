...
- Approval writes the plan to a repo `.md` file, derives the filename from the plan title, and launches `implement-from-plan` in the owning conversation
...
- Add explicit review actions for plan artifacts:
  - `approve` writes the plan content to disk as a markdown file, records the file path on the artifact, creates a conversation-scoped direct flow launch for `implement-from-plan.dot`, and marks the artifact approved
...
- Define deterministic file-writing behavior:
  ...
  - Strip markdown formatting, slugify to a filesystem-safe name, and write at repo root as `<slug>.md`
...
- Backend/service test: approving a plan launches `implement-from-plan.dot` in the artifact’s owning conversation, not the ambient active conversation
- Backend/service test: approval records `written_plan_path`, `flow_launch_id`, and `run_id` on the artifact
...
- Repo-root markdown output is acceptable for v1 unless a dedicated plans directory is later introduced
