...
def test_validate_run_artifact_contract_accepts_minimal_required_status_payload(tmp_path: Path) -> None:
    ...
    errors = validate_run_artifact_contract(logs_root=logs_root, status_node_ids=("plan",))

    assert errors == []


def test_validate_run_artifact_contract_rejects_invalid_optional_field_type(tmp_path: Path) -> None:
    logs_root = tmp_path / "logs"
    (logs_root / "artifacts").mkdir(parents=True, exist_ok=True)
    stage_dir = logs_root / "plan"
    stage_dir.mkdir(parents=True, exist_ok=True)
...
def test_validate_run_artifact_contract_rejects_invalid_preferred_label_type(tmp_path: Path) -> None:
    logs_root = tmp_path / "logs"
    (logs_root / "artifacts").mkdir(parents=True, exist_ok=True)
    stage_dir = logs_root / "plan"
    stage_dir.mkdir(parents=True, exist_ok=True)
