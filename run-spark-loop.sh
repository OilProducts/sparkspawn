...
create_iteration_worktree() {
  ...
  iteration_worktree="$(mktemp -d "${TMPDIR:-/tmp}/spark-loop-worktree.XXXXXX")"
...
5) After implementation, spawn a sub-agent to evaluate whether the item is tested and implemented in the spirit of spark-ui-ux.md and spark-workspace.md.
...
2) Choose one action:
   - If already implemented/tested in spirit of spark-ui-ux.md and spark-workspace.md, mark that Deferred item as `[x]` with a short verification note.
...
4) Checklist gate rules (required):
   ...
   - If you marked `[x]` before evaluation, revert it back to `[ ]` before commit unless verdict is `pass`.
5) Spawn a sub-agent to evaluate whether your action is correct in spirit of spark-ui-ux.md and spark-workspace.md.
   - If your changes touched `frontend/`, you MUST provide screenshot paths from `frontend/artifacts/ui-smoke/` to the sub-agent and ask for visual QA findings from those images.
